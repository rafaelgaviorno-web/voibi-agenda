    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Crie e gerencie os serviços que você oferece.</p>
        </div>
      ...
      <div className="grid gap-4 mt-8">
        {!eventos || eventos.length === 0 ? (
           <div className="p-12 text-center border-2 border-dashed border-zinc-200 dark:border-zinc-700 rounded-xl bg-zinc-50/50 dark:bg-zinc-900/50">
             <p className="text-zinc-500 dark:text-zinc-400 font-medium">Nenhum evento cadastrado.</p>
             <p className="text-sm text-zinc-400 mt-1">Vá em Configurações para criar seu Perfil antes.</p>
           </div>
        ) : null}
        
        {eventos?.map((ev) => (
          <div key={ev.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl p-5 shadow-sm flex items-center justify-between group hover:border-zinc-300 dark:border-zinc-600 transition-colors">
            ...
            <div className="flex items-center gap-2">
               <button className="p-2 text-zinc-400 hover:text-zinc-900 dark:text-zinc-100 rounded-md hover:bg-zinc-100 dark:bg-zinc-800 transition-colors">
