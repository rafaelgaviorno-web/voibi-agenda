  ...
  return (
    <form action={async (formData) => {
      ...
      <div className="space-y-1.5 pb-4 border-b border-zinc-100">
        <label className="text-sm font-medium text-zinc-700">Tipo de Acesso (Papel)</label>
        <div className="flex gap-4 mt-2">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm font-medium text-zinc-800">Usuário Padrão</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input 
              type="radio" 
              ...
              className="text-blue-600 focus:ring-blue-500" 
            />
            <span className="text-sm font-medium text-zinc-800">Administrador (Acesso Total)</span>
          </label>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1.5">
           ...
           <input required name="nome" className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" placeholder="Ex: Ana Souza" />
        </div>
        <div className="space-y-1.5">
           <label className="text-sm font-medium text-zinc-700">WhatsApp</label>
           <input name="whatsapp" className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" placeholder="(00) 00000-0000" />
        </div>
        <div className="space-y-1.5">
           <label className="text-sm font-medium text-zinc-700">E-mail (Login)</label>
           <input required type="email" name="email" className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" placeholder="ana@clinica.com" />
        </div>
        <div className="space-y-1.5">
           <label className="text-sm font-medium text-zinc-700">Senha Provisória</label>
           <input required type="text" name="senha" className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" placeholder="••••••••" />
        </div>
        {unidades && unidades.length > 0 && (
          <div className="space-y-1.5">
             ...
             <select 
               ...
               className="w-full bg-white border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none cursor-pointer"
             >
      ...
      <div className={`space-y-5 transition-opacity ${isAdmin ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="space-y-3">
          <label className="text-sm font-medium text-zinc-700 block border-b border-zinc-100 pb-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {TABS.map(tab => (
              <label key={tab.id} className="flex items-center gap-2 cursor-pointer p-2 bg-zinc-50 rounded-md border border-zinc-200 hover:border-blue-300">
                <input 
                  type="checkbox" 
                  name="abas" 
                  value={tab.id} 
                ...
                />
                <span className="text-sm text-zinc-700 font-medium">{tab.label}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-zinc-50/50 p-4 rounded-lg border border-zinc-100">
            ...
            {agendasFiltradas.map(agenda => (
              <label key={agenda.id} className="flex items-center gap-3 cursor-pointer p-2 hover:bg-white rounded-md transition-colors border border-transparent hover:border-zinc-200">
                <input 
                  type="checkbox" 
                  name="agendas" 
                  value={agenda.id} 
            ...
            {agendas.length === 0 && <div className="text-sm text-zinc-500 col-span-2">Nenhuma agenda cadastrada na clínica.</div>}
          </div>
        </div>
